...
/* XXX: edit from upstream lemon.c to fit token definitions in an enum */
#define TOK_FMT "\t%s%-30s = %3d,\n"
  if( lemp->tokenprefix ) prefix = lemp->tokenprefix;
  else                    prefix = "";
  ...
  if( out ){
    for(i=1; i<lemp->nterminal; i++){
      fprintf(out,TOK_FMT,prefix,lemp->symbols[i]->name,i);
    }
    fclose(out);
  }
  return;
